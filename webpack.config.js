...
const { getLinkedSharedComponentsPath } = require('./jsbuild-utils');

/**
 * PATH VARIABLES & UTILITIES
 */
const spcSymlinkSettings = getLinkedSharedComponentsPath();
const isSymlinked = spcSymlinkSettings.isLinked;

const PATHS = {
    static: path.resolve(__dirname, 'src/encoded/static'),
    build: path.resolve(__dirname, 'src/encoded/static/build'),
    spc: spcSymlinkSettings.sharedComponentPath,
};
...
const rules = [
    ...
    // Build SCSS (include SCSS files)
    {
        test: /\.scss$/,
        use: [
            process.env.NODE_ENV === 'production'
                ? MiniCssExtractPlugin.loader // MiniCssExtractPlugin loader splits css into multiple files for optimized loading
                : 'style-loader', // style-loader injects style tag directly into html for dev hot-reloading
            'css-loader',
            process.env.NODE_ENV !== 'production' && isSymlinked
                ? // If doing a dev or quick build and symlinked, process scss files from SPC, too
                  {
                      loader: 'sass-loader',
                      options: {
                          sassOptions: {
                              includePaths: [path.resolve(PATHS.spc, 'scss')],
                          },
                      },
                  }
                : 'sass-loader', // Otherwise, just load files from this repo
        ],
    },
];
...
const plugins = [
    ...
    // Removes the empty js files generated by webpack when compiling scss to css
    // TODO: Check back every once in a while to see if this has been deprecated/fixed in webpack 5
...
/***
 * FINAL EXPORTS
 * For client and server-side render processes
 */
module.exports = [
    // for browser
    {
        ...
        entry: {
            ...
            style: PATHS.static + '/scss/style.scss',
            print: PATHS.static + '/scss/print.scss',
        },
        target: 'web',
        output: {
            // Handle build of JS bundle
            path: PATHS.build,
            ...
            // Output processed SCSS to CSS folder
            assetModuleFilename: 'static/css/[name].[contenthash][ext][query]', // Output CSS files
            devtoolModuleFilenameTemplate: '[resource-path]', // Sourcemaps put in same folder as outputted CSS
            devtoolFallbackModuleFilenameTemplate: '[resource-path]?[hash]',

            // Settings for both CSS and JS
            sourceMapFilename: '[file].map', // Used for JS and CSS sourcemaps
            libraryTarget: 'umd',
            library: 'App',
            umdNamedDefine: true,
        },

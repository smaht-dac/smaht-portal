    ...
    it('Click & visit each page from menu, ensure ToC exists somewhere, ensure ToC works.', function () {
        cy.on('uncaught:exception', function (err, runnable) {
            ...
            Cypress.log({
                ...
                message:
                    'Hit error re: non-serializable function; fixed in subseq. deploys.',
            });
        ...
        cy.loginSMaHT({
            ...
            .then(() => {
                cy.get(
                    ...
                    const allLinkElementIDs = Cypress._.map(
                        $listItems,
                        function (liEl) {
                            return liEl.id;
                        }
                            ...
                            function testVisit() {
                                function finish(titleText) {
                                    ...
                                    Cypress.log({
                                        name:
                                            ...
                                            $listItems.length,
                                        message:
                                            'Visited page with title "' +
                                            titleText +
                                            '".',
                                    });
                                    if (count < $listItems.length) {
                                        cy.get(
                                            documentationNavBarItemSelectorStr
                                        )
                                            .click()
                                            .should(
                                                'have.class',
                                            ...
                                            .then(() => {
                                                ...
                                                cy.get(
                                                    'div.big-dropdown-menu a#' +
                                                        escapeElementWithNumericId(
                                                            allLinkElementIDs[
                                                                count
                                                            ]
                                                        )
                                ...
                                cy.get('#page-title-container .page-title')
                                    ...
                                    .then((t) => {
                                        ...
                                        expect(titleText).to.have.length.above(
                                            0
                                        );
                                        cy.title()
                                            .should(
                                            ...
                                            if (elements.length > 0) {
                                                cy.get(
                                                    ...
                                                    const textContent = $pre
                                                        .text()
                                                        .trim();
                                                    expect(textContent).to.not
                                                        .be.empty;
                                                });
                                            } else {
                                                cy.log(
                                                    'No <pre> elements found under .rst-container > div'
                                                );
                                            }
                                        });
                                        ...
                                        if (
                                            !haveWeSeenPageWithTableOfContents
                                        ) {
                                            cy.window().then((win) => {
                                                const tocItems =
            ...
            .then(() => {
                ...
                cy.get('.big-dropdown-menu.is-open a.level-2-title').then(
                    ($listItems) => {
                        ...
                        var itemIndicesToVisit = Cypress._.sampleSize(
                                    ...
                                    if (!skipPages.includes(titleText)) {
                                        ...
                                        cy.get(linkSelector)
                                            ...
                                            .then(() => {
                                                ...
                                                if (
                                                    itemIndicesToVisit.length >
                                                    0
                                                ) {
                                                    const nextIndexToVisit =
                                                        itemIndicesToVisit.shift();
                                                    cy.get(
                                                        documentationNavBarItemSelectorStr
                                                    )
                                                        .should(
                                                        ...
                                                        .then(() => {
                                                            cy.get(
                                                                '.big-dropdown-menu.is-open a.level-2-title'
                                                            )
                                                                .eq(
                                                                    nextIndexToVisit
                                                                )
                        ...
                        const firstItemIndexToVisit =
                            itemIndicesToVisit.shift();
                        cy.wrap($listItems.eq(firstItemIndexToVisit))
                            .click()
                            .then(function ($linkElem) {
                                const linkHref = $linkElem.attr('href');
                                cy.location('pathname').should(

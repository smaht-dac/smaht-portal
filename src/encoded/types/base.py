...
def mixin_smaht_permission_types(schema: dict) -> dict:
    ...
    schema["properties"]["submission_centers"] = {
        "type": "array",
        "items": {"type": "string", "linkTo": "SubmissionCenter"},
        "serverDefault": "user_submission_centers",
    }
    schema["properties"]["consortia"] = {
        "type": "array",
        "items": {"type": "string", "linkTo": "Consortium"},
...
class SMAHTCollection(Collection, AbstractCollection):
    ...
    def __init__(self, *args, **kw):
        ...
        if hasattr(self, "__acl__"):
            PRINT(
                f"DEBUG_PERMISSIONS: returning {self.__acl__} for {self.type_info.name}"
            )
            return
        ...
        if "submission_centers" in self.type_info.factory.schema["properties"]:
            PRINT(
                f"DEBUG_PERMISSIONS: returning {ALLOW_SUBMISSION_CENTER_CREATE_ACL} for {self.type_info.name}"
            )
    ...
    def __acl__(self):
        ...
        if "consortia" in properties:
            if DEBUG_PERMISSIONS:
                PRINT(
                    f"DEBUG_PERMISSIONS: Using consortia ACLs status {status} for {self}"
                )
            return self.CONSORTIUM_STATUS_ACL.get(status, ONLY_ADMIN_VIEW_ACL)
        if "submission_centers" in properties:
            if DEBUG_PERMISSIONS:
                PRINT(
                    f"DEBUG_PERMISSIONS: Using submission_centers ACLs status {status} for {self}"
                )
...
def edit(context, request):
    ...
    if request.has_permission("edit"):
        return {
            ...
            "href": "{item_uri}?currentAction=edit".format(
...
def create(context, request):
    if request.has_permission("create"):
        return {
            ...
            "title": "Create",
            "profile": "/profiles/{ti.name}.json".format(ti=context.type_info),
            "href": "{item_uri}?currentAction=create".format(
                item_uri=request.resource_path(context)
            ),
        }

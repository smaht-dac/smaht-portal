...
def validate_unique_page_name(context, request):
    ...
    if "identifier" in data:
        lookup_res = request.registry[CONNECTION].storage.get_by_json(
            "identifier", data["identifier"], "page"
        )
        if lookup_res:
            ...
            if (
                ...
                return
            error_msg = (
                "page %s already exists with name '%s'. This field must be unique"
...
def generate_page_tree(request, page_name=None):
    ...
    if page_name is not None:
        page_name_parts = [
            path_component for path_component in page_name.split("/") if path_component
        ]
        if len(page_name_parts) > 1:
            current_page_route_root = [
                path_component
                for path_component in page_name.split("/")
                if path_component
            ][0]
    ...
    for page in get_iterable_search_results(
        ...
        path_components = [
            path_component
            for path_component in page["identifier"].split("/")
            if path_component
        ]
        if len(path_components) != 1:
            continue
        if (
            current_page_route_root is not None
            and current_page_route_root == page["identifier"]
        ):
...
def generate_at_type_for_page(node):
    capitalized_path_names = [
        pg.capitalize() for pg in filter(lambda pg: pg, node["@id"].split("/"))
    ]
    page_type = []
...
def cleanup_page_tree(node):
    ...
    if node_children_length == 0:
        ...
        node["children"] = [
            ...
            if (len(c.get("children", [])) != 0 or len(c.get("content", [])) != 0)
        ]
...
def is_static_page(info, request):
    ...
    if (
        path_parts[0] != "pages"
        and path_parts[0] in request.registry[COLLECTIONS].keys()
    ):
        return False

    request.set_property(
        lambda x: generate_page_tree(x, page_name), name="_static_page_tree", reify=True
    )

    # NOTE: the below can cause DB connections to leak - we should implement a better lookup - Will 28 March 2024
    request.set_property(
        lambda x: request.registry[CONNECTION].storage.get_by_json(
            "identifier", page_name, item_type="page"
        ),
        name="_static_page_model",
        reify=True,
    )
...
class Page(Item, CorePage):
    ...
    embedded_list = ["content.*", "children.*", "children.children.*"]
...
def static_page(request):
    ...
    def remove_relations_in_tree(node, keep="children"):
        ...
        for field in node.keys():
            if (
                field not in ["next", "previous", "children", "parent"]
    ...
    if tree is not None:
        ...
        for path_idx, part in enumerate(path_parts):
            for child in curr_node.get("children", []):
                ...
                if (
                    len(split_child_name) > path_idx
                    and split_child_name[path_idx] == part
                ):
                    curr_node = child
                    break
    ...
    if context.properties.get("redirect") and context.properties["redirect"].get(
        "enabled"
    ):  # We have a redirect defined.
        parsed_redirect_uri = urlparse(
            context.properties["redirect"].get("target", "/")
        )
        uri_to_use = (
            (parsed_redirect_uri.scheme and (parsed_redirect_uri.scheme + ":") or "")
            + "//"
            if parsed_redirect_uri.netloc
            else ""
        )
        ...
        uri_to_use += (
            ...
            + urlencode(
                {
                    "redirected_from": "/"
                    + context.properties.get("identifier", str(context.uuid))
                }
            )
    ...
    if page_in_tree:
        ...
        if curr_node.get("parent"):
            item["parent"] = remove_relations_in_tree(
                curr_node["parent"], keep="parent"
            )

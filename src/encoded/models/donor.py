...
class Donor(BaseModel):
    ...
    submitted_id: constr(pattern=r'^[A-Z0-9]{3,}_DONOR_[A-Z0-9-_.]{4,}$') = Field(
    ...
    age: conint(ge=0) = Field(..., description='Age of the donor in years', title='Age')
    body_mass_index: Optional[confloat(ge=0.0)] = Field(
        None,
        description='Body mass index of the donor (m/kg^2)',
        title='Body Mass Index',
    )
    height: Optional[confloat(ge=0.0)] = Field(
        None, description='Height of the donor in meters', title='Height'
    )
    sex: Sex = Field(..., description='Sex of the donor', title='Sex')
    weight: Optional[confloat(ge=0.0)] = Field(
        None, description='Weight of the donor in kilograms', title='Weight'
    )

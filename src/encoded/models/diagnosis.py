...
class Diagnosis(BaseModel):
    ...
    submitted_id: constr(pattern=r'^[A-Z0-9]{3,}_DIAGNOSIS_[A-Z0-9-_.]{4,}$') = Field(
    ...
    age_at_diagnosis: Optional[conint(ge=0)] = Field(
        None, description='Age when the disease was diagnosed', title='Age At Diagnosis'
    )
    age_at_resolution: Optional[conint(ge=0)] = Field(
        None,
        description='Age when the disease was determined to have resolved',
        title='Age At Resolution',
    )
    comments: Optional[str] = Field(
        None, description='Additional information on the diagnosis', title='Comments'
    )
    disease: str = Field(
        ..., description='Link to associated disease ontology term', title='Disease'
    )
    ...
    therapeutics: Optional[List[str]] = Field(
        None,

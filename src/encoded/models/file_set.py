...
class FileSet(BaseModel):
    ...
    submitted_id: constr(pattern=r'^[A-Z0-9]{3,}_FILE-SET_[A-Z0-9-_.]{4,}$') = Field(
    ...
    assay: str = Field(..., description='Link to associated assay', title='Assay')
    libraries: List[str] = Field(
        ..., description='Link to associated libraries', min_length=1, title='Libraries'
    )
    sequencing: str = Field(
        ..., description='Link to associated sequencing', title='Sequencing'
    )
    ...
    files: Optional[List[str]] = Field(None, title='Files')

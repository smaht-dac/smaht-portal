...
#   filename:  cell_culture_mixture.json
...
class Component(BaseModel):
    ratio: Optional[confloat(ge=0.0)] = Field(
        None,
        description='Ratio of the cell culture to the total mixture (percentage)',
        title='Ratio',
    )
    cell_culture: Optional[str] = Field(
        None, description='Link to associated cell culture', title='Cell Culture'
    )
...
class CellCultureMixture(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    ...
    submitted_id: constr(
        pattern=r'^[A-Z0-9]{3,}_CELL-CULTURE-MIXTURE_[A-Z0-9-_.]{4,}$'
    ...
    cell_line: Optional[List[str]] = Field(None, title='Cell Lines')

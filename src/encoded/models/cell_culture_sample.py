...
#   filename:  cell_culture_sample.json
...
class CellCultureSample(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    ...
    submitted_id: constr(
        pattern=r'^[A-Z0-9]{3,}_CELL-CULTURE-SAMPLE_[A-Z0-9-_.]{4,}$'
    ...
    cell_density: Optional[confloat(ge=0.0)] = Field(
        None,
        description='The number of cells per mL of storage medium',
        title='Cell Density',
    )
    volume: Optional[confloat(ge=0.0)] = Field(
        None, description='The volume of the sample in mL', title='Volume'
    )

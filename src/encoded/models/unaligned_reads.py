...
class UnalignedReads(BaseModel):
    model_config = ConfigDict(
        extra='forbid',
    )
    ...
    submitted_id: constr(pattern=r'^[A-Z0-9]{3,}_UNALIGNED-READS_[A-Z0-9-_.]{4,}$') = (
    ...
    data_type: Optional[List[DataTypeEnum]] = Field(
        ['Unaligned Reads'], min_length=1, title='Data Type'
    )
    ...
    read_pair_number: Optional[ReadPairNumber] = Field(
        None, description='Read pair number, if paired-end', title='Read Pair Number'
    )
    paired_with: Optional[str] = Field(
        None, description='Link to associated paired-end file, if applicable'
    )

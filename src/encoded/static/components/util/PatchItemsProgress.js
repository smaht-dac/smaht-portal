...
export class PatchItemsProgress extends React.PureComponent {
    ...
    patchItemsProcess(patchPayloads, onComplete, opts) {
        ...
        const checkIfCompleted = () => {
            ...
            if (patchesToComplete === countCompleted) {
                ...
                const patchingPercentageComplete =
                    patchesToComplete === 0
                        ? 0
                        : countCompleted / patchesToComplete;
        ...
        function performRequest([patchURL, itemPatchPayload]) {
            return ajax
                .promise(
                    ...
                    'PATCH',
                    {},
                    JSON.stringify(itemPatchPayload)
                )
        ...
        const countConcurrent = Math.min(
            parallelize ? 5 : 1,
            patchesToComplete
        );
    ...
    patchItems(patchPayloads, callback = null, opts = {}) {
        this.setState(
            { isPatching: true, patchingPercentageComplete: 0 },
            () => {
                setTimeout(ReactTooltip.hide, 50); // Hide still-present tooltips, if any (i.e. button that was clicked)
                console.log(
                    "Generated PATCH '../@@update-project-notes/' payloads - ",
                    patchPayloads
                );
                this.patchItemsProcess(
                    patchPayloads,
                    ({ countCompleted, patchErrors }) => {
                        console.info(
                            'Patching Completed, count Items PATCHed -',
                            countCompleted
                        );
                        this.setState(
                            {
    ...
    render() {
        ...
        const { isPatching, patchingPercentageComplete, patchErrors } =
            this.state;
...
export const ProgressModal = React.memo(function ProgressModal(props) {
    ...
    const percentCompleteFormatted =
        Math.round(patchingPercentageComplete * 1000) / 10;
    const finished = patchingPercentageComplete === 1;
    const errorsLen = patchErrors.length;

    let body;
    if (errorsLen > 0) {
        body = <h5 className="my-0 text-danger">{errorsLen} errors</h5>;
    } else if (finished) {
        body = 'Done';
    } else if (isPatching) {
        body = 'Updating...';
    ...
    return (
        <Modal show onHide={onHide}>
            <Modal.Header closeButton={finished}>
                <Modal.Title>
                    {finished ? 'Update Complete' : 'Please wait...'}
                </Modal.Title>
            </Modal.Header>
            <Modal.Body>
                ...
                <div className="progress">
                    <div
                        ...
                        role="progressbar"
                        style={{ width: percentCompleteFormatted + '%' }}
                        aria-valuenow={percentCompleteFormatted}
                        aria-valuemin="0"
                        aria-valuemax="100"
                    />
                </div>

...
// description/submitted_id -- only organ-side Left/Right, a different
// concept) -- but which half a given *aliquot number* belongs to is a fixed
// fact of the recovery protocol itself (Fig. 2a's own layout, identical for
// every donor). See BIVALVED_TEMPLATE_BY_INTERNAL_CODE/
...
// `tissue_type` string (a backend ontology grouping term), i.e. the backend
// already treats them as one tissue block. The aliquot panel follows that:
// it combines TissueSamples from both sibling Tissue records into one box,
// same as this illustrative fallback shows before any real data has loaded.
...
// TissueSample's real external_id verbatim (item_utils/sample.py's
// get_sample_names -> item.get_external_id) -- it isn't declared in
// file.json's `facets`, but Snovault turns any query param into an
// `embedded.<field>` term filter regardless (snovault's
// search.py:prepare_search_term), so filtering by it here still works.
// Mirrors the donor+tissue link BrowseDonorBase.js builds for the Files stat.
//
...
// submitted that specific File record, which for reprocessed/derived files
// is often a downstream analysis center like "HMS DAC" rather than the
// originating GCC). sequencing_center is the better conceptual match for
// "this GCC's own files", but it is not a required File property, so a
// small fraction of files have it unset despite clearly belonging to a GCC;
// those are silently excluded from this link's results -- a known, accepted
// gap, since no single File field reliably captures "which GCC's aliquot
// this derives from" for every File subtype/pipeline stage.
//
...
// e.g. "NDRI TPC") or a TTD/DAC/OC-suffixed center -- only GCC-suffixed
// centers ever run sequencing, so filtering by a non-GCC center would
// structurally always return zero results; only build the link when the
...
// the originating TissueSample, so per-core-position GCC links can only ever
// surface a fraction of a tissue's files. This mirrors exactly the
// donor+tissue query each page's own file-count fetch already uses
// (TissueView.js adds donorDisplayTitle, TissueTypeView.js's is tissue-only
// across every donor sharing the tissue_type), so the count and the link
// always agree. donorDisplayTitle is optional -- omit it to match
// TissueTypeView.js's donor-independent count.

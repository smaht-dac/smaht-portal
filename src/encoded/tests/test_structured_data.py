...
@contextmanager
def _mocked_refs(refs: Optional[List[str]] = None):
...
def _assert_parse_structured_data(file: str, rows: Optional[List[str]] = None,
                                  expected: Union[dict, list] = {}, debug: bool = False) -> None:
    if rows:
...
def _test_parse_structured_data(file: str, rows: Optional[List[str]] = None,
                                expected: Union[dict, list] = {}, debug: bool = False,
                                noschemas: bool = False,
                                norefs: bool = False,
                                refs: Optional[List[str]] = None) -> None:
    if noschemas:
        if norefs or refs:
            with _mocked_schemas():
                with _mocked_refs(refs):
                    _assert_parse_structured_data(file=file, rows=rows, expected=expected, debug=debug)
        else:
            with _mocked_schemas():
                _assert_parse_structured_data(file=file, rows=rows, expected=expected, debug=debug)
    elif norefs or refs:
...
def test_parse_structured_data_2():
    _test_parse_structured_data(file = f"submission_test_file_from_doug_20231106.xlsx", refs = ["/Consortium/smaht"],
        expected = {
        "FileFormat": [
          {
            "identifier": "fastq",

...
class TestMetadataTSVHelper:
    ...
    def check_type_length(cls, es_testapp, t, expected_count):
        res = es_testapp.post_json("/metadata/", {"type": t})
        tsv = res._app_iter[0]
...
class TestMetadataTSVWorkbook:
    ...
    @pytest.mark.parametrize(
        "field_dict,list_of_names,expected",
        [
            ({"simple": 1}, ["simple"], 1),
            (
                {
            ...
            ({"simple": 1}, ["not_simple", "simple"], 1),
            ({"simple": 1}, ["not_simple"], None),
            ({"simple": {"simple2": 1}}, ["simple.simple2"], 1),
            ({"simple": {"simple2": 1}}, ["simple.simple3", "simple.simple2"], 1),
            ({"simple": {"simple2": {"dict": 1}}}, ["simple.simple2"], None),
            ({"simple": {"simple2": ["array"]}}, ["simple.simple2"], "array"),
            (
            ...
            (
                {"simple": {"simple2": [{"key": "val1"}, {"key": "val2"}]}},
                ["simple.simple2.key"],
                "val1|val2",
            ),
            (
                {
                    "simple": {
                        "simple2": {"simple3": [{"key": "val1"}, {"key": "val2"}]}
                    }
                },
                ["simple.simple2.simple3.key"],
                "val1|val2",
            ),
        ],
    )
    ...
    def test_metadata_tsv_workbook(self, workbook, es_testapp):
        ...
        res = es_testapp.post_json(
            "/metadata/", {"type": "File", "include_extra_files": True}
        )
        tsv = res._app_iter[0]
        ...
        TestMetadataTSVHelper.check_key_and_length(
            header2, "Suggested command to download: "
        )
        TestMetadataTSVHelper.check_key_and_length(header3, "FileDownloadURL")
        assert (
            len(parsed[3:]) == 14
        )  # there are 14 entries in the workbook right now, including extra files
        ...
        TestMetadataTSVHelper.check_type_length(es_testapp, "GeneralFile", 2)

        res = es_testapp.post_json(
            "/metadata/", {"type": "OutputFile", "include_extra_files": True}
        )
        tsv = res._app_iter[0]
        parsed = TestMetadataTSVHelper.read_tsv_from_bytestream(tsv)
        ...
        for row in parsed:
            if "303985cf-f1db-4dea-9782-2e68092d603d" in row[0]:  # this is the row
                assert (
                    row[2]
                    == "SMHT-FOO-BAR-M45-B003-DAC_SMAURF3ETDQJ_bwamem0.1.2_GRCh38.aligned.sorted.bam"
                )
                ...
                assert (
                    row[17]
                    == "smaht-TEST_TISSUE_LIVER-illumina_novaseqx-Paired-end-150-R9-bulk_wgs"
                )  # merge grp
                break
    ...
    def test_peak_metadata_workbook(self, workbook, es_testapp):
        ...
        # check all types
        res = es_testapp.post_json(
            "/peek-metadata/", {"type": "File", "include_extra_files": False}
        ).json
        for facet in res:
            if facet["field"] == "file_size":
                assert (
                    facet["count"] == 10
                )  # here are 10 entries in the workbook with file_size
        ...
        res = es_testapp.post_json(
            "/peek-metadata/", {"type": "OutputFile", "include_extra_files": True}
        ).json
        for facet in res:
            if facet["field"] == "extra_files.file_size":

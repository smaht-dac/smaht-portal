...
@pytest.mark.parametrize(
    "patch_body,expected_status", [
        ...
        ({"read_pair_number": "R1"}, 422),
        ({"paired_with":"TEST_UNALIGNED-READS_FASTQ_NO_READ_PAIR_NUMBER"}, 422),
...
@pytest.mark.parametrize(
    "post_body,expected_status,index", [
        ({"read_pair_number": "R2","paired_with":"TEST_UNALIGNED-READS_FASTQ"}, 201, 1),
        ({"read_pair_number": "R1","paired_with":"TEST_UNALIGNED-READS_FASTQ"}, 422, 2),
        ({"paired_with":"TEST_UNALIGNED-READS_FASTQ"}, 422, 3),
        ({"read_pair_number": "R2","paired_with":"TEST_UNALIGNED-READS_FASTQ_NO_READ_PAIR_NUMBER"}, 422, 4),
        ({"read_pair_number":"R1"}, 201, 5),
        ({"read_pair_number": "R2"}, 422, 6),
        ({}, 201, 7),
    ]
)

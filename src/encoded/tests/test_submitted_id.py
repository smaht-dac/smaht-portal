...
VALID_CELL_LINE_SUBMITTED_ID = f"{TEST_SUBMISSION_CENTER_CODE}_CELL-LINE_SOME-IDENTIFIER"


@pytest.fixture
def cell_line_properties(testapp: TestApp, test_submission_center: Dict[str, Any]) -> Dict[str, Any]:
    return {
...
def test_submitted_id_validation_on_post(
    submitted_id: str, expected_status: int, testapp: TestApp,
...
@pytest.mark.parametrize(
    ...
    [
        ("", 401),
        ("SOME-CODE_NOT-CELL-LINE_SOME-IDENTIFIER", 401),
        ("SOME-CODE_CELL-LINE_SOME-IDENTIFIER", 401),
        (VALID_CELL_LINE_SUBMITTED_ID, 201),
    ]
)

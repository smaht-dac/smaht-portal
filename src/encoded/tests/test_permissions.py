...
@pytest.fixture
def submission_center_file(
    testapp, fastq_format, test_submission_center, test_consortium
):
    item = {
...
def protected_file(testapp, fastq_format, test_protected_consortium):
    item = {
        ...
        "data_type": ["Unaligned Reads"],
        "status": "released",  # this status is important as this will make it viewable by consortium
        "consortia": [test_protected_consortium["uuid"]],
    }
    res = testapp.post_json("/OutputFile", item)
    return res.json["@graph"][0]
...
class TestAdminPermissions:
    ...
    def test_admin_can_purge(testapp, fastq_format):
        testapp.patch_json(
            f'/{fastq_format["uuid"]}', {"status": "deleted"}, status=200
        )
    ...
    def test_non_admin_cannot_purge(submitter_testapp, fastq_format):
        submitter_testapp.patch_json(
            f'/{fastq_format["uuid"]}', {"status": "deleted"}, status=422
        )
        submitter_testapp.delete_json(
            f'/{fastq_format["uuid"]}/?purge=True', {}, status=403
        )
...
class TestPermissionsHelper:
    ...
    @classmethod
    def validate_get_permissions(
        cls,
        *,
        restricted_app,
        restricted_expected_status,
        admin_app,
        admin_expected_status,
        anon_app,
        anon_expected_status,
        item_uuid,
    ):
...
class TestSubmissionCenterPermissions(TestPermissionsHelper):
    ...
    def test_submission_center_file_permissions_view(
        ...
        self.validate_get_permissions(
            restricted_app=submission_center_user_app,
            restricted_expected_status=200,
            admin_app=testapp,
            admin_expected_status=200,
            anon_app=anontestapp,
            anon_expected_status=403,
            item_uuid=uuid,
        )

    def test_submission_center_user_permissions_view(
        self,
        submission_center_user_app,
        smaht_gcc_user,
        testapp,
        ...
        testapp.patch_json(
            f'/{smaht_gcc_user["uuid"]}?delete_fields=submission_centers',
            {},
            status=200,
        )
        self.validate_get_permissions(
            restricted_app=submission_center_user_app,
            restricted_expected_status=403,
            admin_app=testapp,
            admin_expected_status=200,
            anon_app=anontestapp,
            anon_expected_status=403,
    ...
    def test_submission_center_user_permissions_cannot_view(
        self,
        submission_center_user_app,
        smaht_gcc_user,
        testapp,
        ...
        uuid = submission_center_file["uuid"]
        testapp.patch_json(
            f"/{uuid}", {"submission_centers": [test_second_submission_center["uuid"]]}
        )
        self.validate_get_permissions(
            restricted_app=submission_center_user_app,
            restricted_expected_status=403,
    ...
    def test_submission_center_user_can_view_consortium_item(
        self, submission_center_user_app, testapp, anontestapp, consortium_file
    ):
    ...
    def test_submission_center_user_cannot_create_other(
        ...
        submission_center_user_app.post_json(
            "/SubmissionCenter", {"title": "dummy", "name": "dummy"}, status=422
        )  # blocked by restricted_fields on required fields
    ...
    def test_submission_center_can_edit_file(
        test_submission_center,
        submission_center_user_app,
        released_file,
        testapp,
    ...
    @pytest.mark.parametrize(
        "new_status",
        ["released", "obsolete", "archived", "deleted", "open", "protected-network"],
    )
    def test_submission_center_cannot_edit_file(
        test_submission_center,
        submission_center_user_app,
        released_file,
        testapp,
    ...
    @staticmethod
    def test_submission_center_user_lacking_submits_for_cannot_create_or_edit(
        test_submission_center,
        test_second_submission_center,
        submission_center_user_app,
        released_file,
        ...
        submission_center_user_app.post_json(
            "/Donor",
            {  # cannot submit a donor under a diferrent center
                "age": 37,
                "sex": "Female",
                "submission_centers": [test_second_submission_center["uuid"]],
    ...
    @staticmethod
    def test_mixed_submission_center_no_write_overlap(
        test_submission_center,
        test_second_submission_center,
        submission_center_user_app,
        ...
        submission_center_user_app.post_json(
            "/Donor",
            {  # cannot submit a donor on opposing center
                "age": 37,
                "sex": "Female",
                "submission_centers": [test_second_submission_center["uuid"]],
                "submitted_id": "SECONDTEST_DONOR_ABCD",
        ...
        submission_center2_user_app.post_json(
            "/Donor",
            {  # cannot submit a donor on opposing center
                "age": 37,
                "sex": "Female",
    ...
    def test_mixed_submission_center_no_edit_overlap(
        test_submission_center,
        test_second_submission_center,
        submission_center_user_app,
        submission_center2_user_app,
        ...
        submission_center_user_app.patch_json(
            f"/{atid}", {}, status=200
        )  # matching submission center
        submission_center2_user_app.get(
            f"/{atid}", status=403
        )  # get will not work since not in released status
        submission_center2_user_app.patch_json(
            f"/{atid}", {}, status=422
        )  # different submits_for

    @staticmethod
    @pytest.mark.parametrize("new_status", ["released", "open", "protected-network"])
    def test_mixed_submission_center_can_view_but_not_edit(
        test_submission_center,
        test_second_submission_center,
        submission_center_user_app,
        submission_center2_user_app,
    ...
    @staticmethod
    def test_dbgap_group_with_restricted_status(
        submission_center_user_app,
        submission_center2_user_app,
        testapp,
        protected_consortium_user_app,
        released_file,
        anontestapp,
        ...
        smaht_public_dbgap_app.get(
            f"/{atid}@@download", status=403
        )  # public dbGaP user can't aaccess
        protected_consortium_user_app.get(f"/{atid}@@download", status=307)
        testapp.get(f"/{atid}@@download", status=307)  # admin as well

    @staticmethod
    def test_dbgap_group_with_public_restricted_status(
        submission_center_user_app,
        submission_center2_user_app,
        testapp,
        protected_consortium_user_app,
        ...
        protected_consortium_user_app.get(f"/{atid}@@download", status=307)
        smaht_public_dbgap_app.get(
            f"/{atid}@@download", status=307
        )  # public dbGaP user can access
        testapp.get(f"/{atid}@@download", status=307)  # admin as well
...
class TestConsortiumPermissions(TestPermissionsHelper):
    ...
    def test_consortium_file_permissions_view(
        self,
        consortium_user_app,
        smaht_consortium_user,
        testapp,
        anontestapp,
        consortium_file,
    ):
        ...
        uuid = consortium_file["uuid"]
        self.validate_get_permissions(
            restricted_app=consortium_user_app,
            restricted_expected_status=200,
            admin_app=testapp,
            admin_expected_status=200,
            anon_app=anontestapp,
    ...
    def test_consortium_user_permissions_view(
        self,
        consortium_user_app,
        smaht_consortium_user,
        testapp,
        ...
        testapp.patch_json(
            f'/{smaht_consortium_user["uuid"]}?delete_fields=consortia', {}, status=200
        )
        self.validate_get_permissions(
            restricted_app=consortium_user_app,
            restricted_expected_status=403,
    ...
    def test_consortium_user_cannot_create_other(
        test_submission_center, consortium_user_app, smaht_consortium_user, testapp
    ):
        ...
        consortium_user_app.post_json(
            "/Image", {"description": "test image", "status": "draft"}, status=422
        )  # blocked by restricted_fields on required fields
        consortium_user_app.post_json(
            "/SubmissionCenter", {"title": "dummy", "name": "dummy"}, status=422
        )  # blocked by restricted_fields on required fields
    ...
    def test_consortium_user_cannot_view_submission_center_data(
        submission_center_file, new_status, testapp, consortium_user_app
    ):
    ...
    @staticmethod
    def test_consortium_user_can_view_public_submission_center_data(
        submission_center_file, consortium_user_app, testapp
    ):
    ...
    @staticmethod
    @pytest.mark.parametrize(
        "new_status", ["released", "obsolete", "open", "protected-network"]
    )
    def test_consortium_user_can_view_dual_tagged_data(
        released_file, consortium_user_app, new_status, testapp
    ):
    ...
    def test_consortium_user_cannot_view_or_edit_dual_tagged_data(
        released_file, consortium_user_app, new_status, testapp
    ):
...
class TestAnonUserPermissions:
    ...
    @staticmethod
    def test_public_user_can_view_public_data(
        released_file, testapp, anontestapp, unassociated_user_app
    ):
    ...
    def test_public_user_cannot_view_restricted_data(
        released_file, new_status, testapp, anontestapp, unassociated_user_app
    ):
...
class TestProtectedDataPermissions:
    ...
    * User that is part of 2 submission centers and both consortia
    ...
    def test_controlled_user_can_access_controlled_data(
        testapp,
        protected_file,
        consortium_file,
        protected_consortium_user_app,
    ...
    def test_controlled_user_cannot_access_controlled_data(
        testapp,
        protected_file,
        protected_consortium_user_app,
        new_status,
    ...
    def test_controlled_user_cannot_access_submitter_data(
        testapp, released_file, protected_consortium_user_app, new_status
    ):
    ...
    def test_submitter_user_can_access_submitter_data(
        testapp, released_file, protected_consortium_submitter_app, new_status
    ):
...
class TestUserSubmissionConsistency:
    ...
    @staticmethod
    def test_user_submission_center_consistency(
        testapp,
        test_consortium,
        test_protected_consortium,
        test_submission_center,
        test_second_submission_center,
        submission_center_user_app,
    ):
    ...
    @staticmethod
    def test_user_consortia_submission_consistency_alternate_id(
        testapp, test_consortium, test_protected_consortium, consortium_user_app
    ):
    ...
    @staticmethod
    def test_user_submission_center_consistency_alternate_id(
        testapp,
        test_submission_center,
        test_second_submission_center,
        submission_center_user_app,
        ...
        submission_center_user_app.post_json(
            ...
            {
                "title": "test",
                "submission_centers": [test_second_submission_center["@id"]],
            },
            status=422,
        )
...
def test_user_can_view_profile(
    protected_consortium_user_app: TestApp,
    smaht_consortium_protected_user: Dict[str, Any],
) -> None:
    """Tests that a non-admin user can view their own profile"""
    assert protected_consortium_user_app.get(
        f'{smaht_consortium_protected_user["@id"]}', status=200
    )
...
def test_item_create_permissions(
    ...
    for item_type in loadxl_order:
        ...
        for item in test_properties:
            item = fix_sample_submitted_ids_for_tests(item)
...
def assert_expected_special_permissions(
    ...
    if item_type == "access_key":
        ...
        raise NotImplementedError(f"Special item type {item_type} not handled.")
...
@pytest.fixture
def protected_donor_restricted_medical_history(
    testapp, protected_donor, test_submission_center
):
    item = {
...
def test_protected_donor_restricted_view(
    protected_donor,
    protected_donor_restricted_medical_history,
    unassociated_user_app: TestApp,
    submission_center_user_app: TestApp,
    consortium_user_app: TestApp,
...
@pytest.fixture
def protected_donor_public_restricted_medical_history(
    testapp, public_protected_donor, test_submission_center
):
    item = {
        "uuid": "14557335-dc8f-417f-8e3e-c0624f795897",
        "submitted_id": "TEST_MEDICAL-HISTORY_FEMALE",
...
def test_protected_donor_public_restricted_view(
    public_protected_donor,
    protected_donor_public_restricted_medical_history,
    unassociated_user_app: TestApp,
    submission_center_user_app: TestApp,
    consortium_user_app: TestApp,
    smaht_dbgap_app: TestApp,
    ...
    public_protected_medical_history = (
        protected_donor_public_restricted_medical_history["uuid"]
    )
    for user_app in [
        unassociated_user_app,
        submission_center_user_app,
        ...
        user_app.get(f"/protected-donors/{donor_uuid}/", status=403)
        user_app.get(
            f"/medical-histories/{public_protected_medical_history}/", status=403
        )

    # dbGaP group users can
    smaht_public_dbgap_app.get(f"/protected-donors/{donor_uuid}/", status=200)
    smaht_public_dbgap_app.get(
        f"/medical-histories/{public_protected_medical_history}/", status=200
    )
    smaht_dbgap_app.get(f"/protected-donors/{donor_uuid}/", status=200)
    smaht_dbgap_app.get(
        f"/medical-histories/{public_protected_medical_history}/", status=200
...
def test_public_user_retains_submission_center_on_patch(testapp, blank_user):
    ...
    item = testapp.patch_json(
        f'/{blank_user["uuid"]}', {"status": "current"}  # empty patch in effect
    ).json["@graph"][0]
